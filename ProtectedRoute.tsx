...
import type { Appointment, Condition } from '../lib/types'
...
export default function AppointmentsPage() {
  ...
  const load = async () => {
    ...
    setAppointments(aptRes.data ?? []); setConditions(condRes.data ?? []); setLoading(false)
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    await supabase.from('appointments').delete().eq('id', id).eq('user_id', user.id); load()
  }
          ...
          <div className="bg-white rounded-2xl p-8 w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input type="text" required value={form.doctor_name} onChange={(e) => setForm({ ...form, doctor_name: e.target.value })}
                  className="w-full px-4 py-3 text-lg border border-neutral-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="e.g., Dr. Smith" />
              </div>
              <div>
                ...
                <input type="text" value={form.specialty} onChange={(e) => setForm({ ...form, specialty: e.target.value })}
                  className="w-full px-4 py-3 text-lg border border-neutral-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="e.g., Cardiologist" />
              </div>
              <div>
                <label className="block text-base font-medium text-neutral-700 mb-2">Location</label>
                <input type="text" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })}
                  className="w-full px-4 py-3 text-lg border border-neutral-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="e.g., City Hospital, Room 204" />
              </div>
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })}
                  className="w-full px-4 py-3 text-lg border border-neutral-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" rows={2} placeholder="Any preparation or questions for the doctor" />
              </div>
            ...
            <div>
              ...
              <div className="space-y-4">
                {upcoming.map((a) => {
                  ...
                  return (
                    <div key={a.id} className="bg-white rounded-2xl border border-neutral-200 p-6 hover:shadow-md transition-shadow">
          ...
          {past.length > 0 && (
            <div>
              ...
              <div className="space-y-3">
                {past.map((a) => (
                  <div key={a.id} className="bg-white rounded-2xl border border-neutral-200 p-5 opacity-60">
                    <div className="flex items-center justify-between">
                      <div>

...
const emptyForm = { name: '', description: '', diagnosed_date: '', severity: 'moderate' as Severity }
...
export default function ConditionsPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!user || !confirm('Are you sure you want to delete this condition?')) return
    await supabase.from('conditions').delete().eq('id', id).eq('user_id', user.id); load()
  }
    ...
    <div className="space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                  className="w-full px-4 py-3 text-lg border border-neutral-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="e.g., Diabetes Type 2" />
              </div>
              <div>
                <label className="block text-base font-medium text-neutral-700 mb-2">Description</label>
                <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
                  className="w-full px-4 py-3 text-lg border border-neutral-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" rows={3} placeholder="Describe your condition and any important notes" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-base font-medium text-neutral-700 mb-2">Diagnosed Date</label>
                  <input type="date" value={form.diagnosed_date} onChange={(e) => setForm({ ...form, diagnosed_date: e.target.value })}
      ...
      {conditions.length === 0 ? (
        ...
        <div className="space-y-4">
          {conditions.map((c) => (
            <div key={c.id} className="bg-white rounded-2xl border border-neutral-200 p-6 hover:shadow-md transition-shadow">
